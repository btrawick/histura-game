...
type RawAll = Partial<Record<Relationship, RawBySide>>;
...
function bake(
  raw: RawAll
): Record<Relationship, Record<RelationSide, RelationQuestion[]>> {
  const out: Partial<
    Record<Relationship, Record<RelationSide, RelationQuestion[]>>
  > = {};

  (Object.keys(raw) as Relationship[]).forEach((rel) => {
    const bySide = raw[rel];
    if (!bySide) return;

    out[rel] = { p1: [], p2: [] };
    (['p1', 'p2'] as RelationSide[]).forEach((side) => {
      out[rel]![side] = bySide[side].map((q, i) => ({
  ...
  // We assert full coverage here because extraQuestions fills the gaps.
  return out as Record<Relationship, Record<RelationSide, RelationQuestion[]>>;
}
...
// Base baked questions
export const questions = bake(raw);

// Merge in extra pairs (kid-kid, sibling-sibling, etc.)
