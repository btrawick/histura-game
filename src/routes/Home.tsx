...
export default function Home() {
  ...
  const { relationship, setRelationship, players, setPlayer, swapPlayers, startNewGame } = useGame();
    ...
    <div className="container">
      ...
      <div className="card" style={{ marginTop: 16, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <div style={{ display: 'grid', gap: 8 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px', gap: 8, alignItems: 'center' }}>
            <select value={toTitle(players.p1.role)} disabled><option>{toTitle(players.p1.role)}</option></select>
            <AvatarPreview name={players.p1.name || labels.p1} dataUrl={players.p1.avatarDataUrl} onClick={() => setCaptureFor('p1')} />
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gap: 8 }}>
          ...
          />
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px', gap: 8, alignItems: 'center' }}>
            <select value={toTitle(players.p2.role)} disabled><option>{toTitle(players.p2.role)}</option></select>
            <AvatarPreview name={players.p2.name || labels.p2} dataUrl={players.p2.avatarDataUrl} onClick={() => setCaptureFor('p2')} />
          </div>
        </div>

        <div style={{ gridColumn: '1 / -1', display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center' }}>
          <div>
            <button className="button secondary" onClick={swapPlayers} title="Swap players (including roles)">Swap players</button>
          </div>
          <button
            className="button"
            onClick={() => { setRelationship(rel); startNewGame(); navigate('/play'); }}
      ...
      {captureFor && (
        <AvatarCapture
          ...
          onSave={(dataUrl) => { setPlayer(captureFor, { avatarDataUrl: dataUrl }); setCaptureFor(null); }}
        />
      )}
    </div>
  );
...
function AvatarPreview({ name, dataUrl, onClick }: { name: string; dataUrl?: string; onClick?: () => void }) {
  const src = dataUrl || `https://api.dicebear.com/9.x/fun-emoji/svg?seed=${encodeURIComponent(name || 'player')}`;
  return (
    <img
      src={src}
      width={72} height={72} alt="avatar" onClick={onClick}
      style={{ borderRadius: 12, width: 72, height: 72, objectFit: 'cover', background: '#0b1220', cursor: onClick ? 'pointer' : 'default' }}
